...
from keyboards.keyboards import (start_kb,
                                 ...
                                 adults_kb,
                                 hotels_count_kb)
from states.states import FSMlowprice, FSMchoice
...
@router.callback_query(Text(text='lowprice'))
async def process_lowprice_answer(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(text=LEXICON['lowprice_greetings'],
                                     reply_markup=cancel_kb)

    await callback.message.answer(text=LEXICON['enter_city'])
    await state.set_state(FSMlowprice.fill_city)
...
@router.message(FSMlowprice.fill_city, check_city_name)
async def process_enter_city(message: Message, state: FSMContext):
    ...
    user_dict[user_id]['search_command'] = 'lowprice'

    await state.set_state(FSMlowprice.fill_adults)


@router.message(FSMlowprice.fill_city)
async def wrong_city(message: Message):
    await message.answer(text=LEXICON['city_name_wrong'])
    await message.delete()


@router.callback_query(FSMlowprice.fill_adults, Text(text=['one', 'two', 'three', 'four']))
async def process_enter_adults(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(FSMlowprice.fill_hotels_count)

@router.message(FSMlowprice.fill_adults)
async def wrong_adults(message: Message):

   await message.answer(text=LEXICON['wrong_adults'])


@router.callback_query(FSMlowprice.fill_hotels_count, Text(text=num_list))
async def process_enter_count(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(FSMlowprice.fill_photo)


@router.message(FSMlowprice.fill_hotels_count)
async def wrong_hotels_count(message: Message):
    await message.answer(text=LEXICON['hotels_count_wrong'])
    await message.delete()


@router.callback_query(FSMlowprice.fill_photo, Text(text=['yes', 'no']))
async def process_enter_photo(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.data == 'yes':
        ...
        await state.set_state(FSMlowprice.fill_photo_count)
        ...
        await state.set_state(FSMlowprice.fill_ckeckIn)




@router.message(FSMlowprice.fill_photo)
async def wrong_get_photo(message: Message):
    await message.answer(text=LEXICON['get_photo_wrong'])
    await message.delete()

@router.callback_query(FSMlowprice.fill_photo_count, Text(text=['three', 'five', 'seven']))
...
async def process_enter_checkIn(message: Message, state: FSMContext):
    ...
    await state.set_state(FSMlowprice.fill_ckeckOut)

@router.message(FSMlowprice.fill_ckeckIn)
async def wrong_checkIn(message: Message):
    await message.answer(text=LEXICON['wrong_date'])


@router.message(FSMlowprice.fill_ckeckOut, check_date, check_check_out)
...
@router.message(FSMlowprice.fill_ckeckOut)
async def wrong_checkIn(message: Message):
    await message.answer(text=LEXICON['wrong_date'])
